...
Open a part/assembly's Drawings tab to add Machining, Inspection, or Assembly drawing records, record working versions, and assess required drawing types. **Approve drawing** records Checked by, Approved by/Engineering participants, notes, and an exact version/model association. DWG revision advances once per newly approved drawing version. **Confirm still applicable** can retain an approved DWG revision for another exact model with documented review. If drawing content or printed PRT REV changes, create and approve a new working version instead.

Future part releases require all drawing types assessed, reasons for Not required types, and current approval of each Required drawing for the exact model being released. Blockers appear in the ECR/OCR Parts & assemblies tab. Finish drawing reviews before final Engineering approval. Existing releases and revision history are preserved. These checks use local metadata; CAD file verification, Drive publication, and authenticated sign-off are not connected.

Create a draft part or assembly from an active ECR/OCR's Parts & assemblies section; the app assigns its internal ID and links the source automatically. Use **Change existing part** to choose an exact source revision, revise the same identity or create an independent variant, and identify intended systems. The sidebar's PDM Library is for browsing records, revision history, drawing records, and activity.

The Parts & assemblies tab supports **Record Engineering approval**, followed by **Record release**. Approval captures meeting details, Ops numbering decisions, and intended systems. Release assigns the next whole revision number while preserving original working history and drawing associations. Parallel ECRs can continue independently: the first approved branch may release; another branch based on older work then shows **Newer revision released** and must document its review before release. Variants have independent release sequences. Odoo number assignment alone never advances a revision.

These approvals and releases are prototype metadata records in this browser. Names and authorization are manually entered; no CAD files are published, drawings approved, or machine configurations changed. Shared authentication, file-backed approval checks, and controlled Drive publication remain future work. Existing browser data upgrades without resetting it. See `PDM_DESIGN.md` for current rules and limitations; its September 25 parallel-release rules supersede the earlier planning PDF.
